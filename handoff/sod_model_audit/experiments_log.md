...
## SOD_MODEL_AUDIT - Lite-SOD 分析审计包

- 类型：模型结构审计与轻量化分析材料
- 输出目录：`handoff/sod_model_audit/`
- 压缩包：`handoff/sod_model_audit.zip`
- 内容范围：当前 full SOD / no-PIoU / batch8 full 配置、模型 YAML、自定义 GFPN-lite / C2f-EMA / EMA / PIoU 代码、关键 results.csv、model.info 输出和审计说明。
- 复杂度摘要：`models/yolov8s_gfpn_lite.yaml` 为 152 layers、12,080,440 params、38.2 GFLOPs，Detect stride 为 4/8/16/32。
- 排除内容：`datasets/`、完整 `runs/`、权重、cache、`.git/`、`.venv/`、`__pycache__/`。
- 用途：供后续 Lite-SOD-A/B/C 判断通道压缩、neck/head 简化、PIoU 去留和 Detect 分支风险。
